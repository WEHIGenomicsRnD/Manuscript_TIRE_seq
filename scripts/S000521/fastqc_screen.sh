...
RUN="S000521";
CDP_DIR="/vast/scratch/users/brown.d/"${RUN}"/fastq/CDP/*.gz";
TIRE_DIR="/vast/scratch/users/brown.d/"${RUN}"/fastq/TIRE/*.gz";
OUTPUT_DIR="/vast/scratch/users/brown.d/"${RUN}"/fastq/Fastqc";
OUTPUT_DIR_FQS="/vast/scratch/users/brown.d/"${RUN}"/fastq/FastqScreen";
READ1=${TIRE_DIR}"/*_R1_001.fastq.gz"
...
fastqc --outdir $OUTPUT_DIR \
 --threads 12 \
 /stornext/Projects/GenomicsRnD/brown.d/S000521/fastq/TIRE/;
