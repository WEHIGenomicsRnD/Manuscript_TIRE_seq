---
title: "pDC timecourse"
description: "pDC timecourse from total culture"
author: "Daniel Brown"
date: "`r Sys.Date()`"
output:
  html_document:
...
# pDC timecourse from total culture
...
Interested in only the total DC cultures and pDCs. Remove the other cell types.

```{r remove_samples}
keep_samples <- c("Total_DC_culture", "pDC", "pre-pDC_like")
...
# Look at timecourse of differntiation from total DC culture to pDC
...
Use a cubic regression spline curve with 2 degrees of freedom.
...
There are 3,688 differentially expressed genes that change over the 7 days.
...
* Cd7
  + CD7 is primarily associated with T cells
  + Not much evidence for expression in pDCs
  + It is expressed in a rare cancer: Blastic Plasmacytoid Dendritic Cell Neoplasm
* Ctss Cathepsin S 
  + Cathepsin S (CTSS) plays an important role in antigen processing and presentation in dendritic cells:
  + CTSS is one of the major antigen-processing enzymes regulating class II presentation in conventional DCs (cDCs)5.
  + In general, immature and mature thymic cDCs express higher levels of CTSS than immature splenic cDCs5.
  + CTSS expression is increased in both thymic and splenic plasmacytoid DCs (pDCs)
* Gm34680
  + Unknown
* Mpo Myeloperoxidase
  + MPO inhibits the generation of adaptive immunity by suppressing DC function
  + MPO interacts with DCs in vivo and inhibits DC activation through enzymatically dependent mechanisms involving reactive intermediates3.
  + MPO inhibits antigen uptake and processing by DCs
